...
it("delegates evaluation creation to the authenticated governance proxy", async () => {
  const request = new Request("http://localhost/api/evaluations", { method: "POST", body: "{}" });
  ...
  expect(proxyGovernanceRequest).toHaveBeenCalledWith(request, "/v1/evaluations");
});
