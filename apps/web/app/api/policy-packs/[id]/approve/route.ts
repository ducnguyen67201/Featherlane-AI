...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(`${API_URL}/v1/policy-packs/${encodeURIComponent(id)}/approve`, {
      method: "POST",
    ...
    return NextResponse.json(
      { detail: "The governance API is unavailable; the policy pack was not published." },
      { status: 503 },
    );
  }
}
