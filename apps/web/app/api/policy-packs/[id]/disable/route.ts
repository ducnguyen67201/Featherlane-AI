...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(`${API_URL}/v1/policy-packs/${encodeURIComponent(id)}/disable`, {
      method: "POST",
    ...
    return NextResponse.json(
      { detail: "The governance API is unavailable; the policy pack was not disabled." },
      { status: 503 },
    );
  }
}
